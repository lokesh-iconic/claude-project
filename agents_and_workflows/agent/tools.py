"""
agent/tools.py — Tool definitions for the agentic support ticket system.

These tools are registered as JSON-schema tool definitions for the Anthropic API.
Each tool wraps a focused sub-prompt and returns structured JSON.

The key difference from the workflow: the AGENT decides which tools to call and
in what order. Tools are capabilities, not pipeline steps.
"""
...
TOOL_EXECUTORS = {
    ...
    # escalate_to_specialist is handled by subagent.py
}
